...
for i = 1:numel(panel_conditions)
    ...
    experimental_color = hex_color(condition.style.experimental_color);
    experimental_face_color = marker_face_color( ...
        condition.style.experimental_marker_face_color);
    ...
    if cfg.figures.show_raw_simulation
        ...
        numerical_handles(end + 1) = numerical_legend_marker( ...
            ax, condition, color, cfg.figures); %#ok<AGROW>
        numerical_labels{end + 1} = sprintf( ...
            'Numerical aggregates: %s', condition.label); %#ok<AGROW>
    end
    ...
    if fit_record.enabled
        [fit_handles(end + 1), fit_labels{end + 1}] = ...
            plot_population_fit(ax, fit_record, condition, color, ...
            cfg.figures); %#ok<AGROW>
    end
    ...
    if cfg.figures.show_tem_error_bars && ...
        ...
        experiment_handles(end + 1) = errorbar(ax, experiment.da_nm, ...
            ...
            'LineStyle', 'none', 'Color', experimental_color, ...
            ...
            'MarkerFaceColor', experimental_face_color, ...
            'LineWidth', condition.style.experimental_marker_line_width); %#ok<AGROW>
    else
        experiment_handles(end + 1) = scatter(ax, experiment.da_nm, ...
            experiment.dbarpp_nm, condition.style.experimental_marker_size, ...
            experimental_color, condition.style.experimental_marker, ...
            'MarkerEdgeColor', experimental_color, ...
            'MarkerFaceColor', experimental_face_color, ...
            'LineWidth', condition.style.experimental_marker_line_width); %#ok<AGROW>
    end
    experiment_labels{end + 1} = sprintf( ...
        'TEM measurements: %s', condition.label); %#ok<AGROW>
...
for i = 1:numel(panel_conditions)
    ...
    experiment_handles(end + 1) = scatter(ax, ...
        ...
        condition.style.experimental_marker_size, experimental_color, ...
        condition.style.experimental_marker, ...
        'MarkerEdgeColor', experimental_color, ...
        'MarkerFaceColor', experimental_face_color, ...
        'LineWidth', condition.style.experimental_marker_line_width); %#ok<AGROW>
    experiment_labels{end + 1} = sprintf( ...
        'AAC-SMPS measurements: %s', condition.label); %#ok<AGROW>
...
function [handle, label] = plot_population_fit(ax, fit_record, ...
        condition, color, figure_cfg)
%PLOT_POPULATION_FIT Draw the fitted trend for a numerical population.
...
label = sprintf('Fit to numerical population: %s', condition.label);

end

function handle = numerical_legend_marker(ax, condition, color, figure_cfg)
%NUMERICAL_LEGEND_MARKER Add a visible proxy for the translucent point cloud.

handle = scatter(ax, NaN, NaN, ...
    max(condition.style.simulation_marker_size, 30), color, ...
    condition.style.simulation_marker, 'filled', ...
    'MarkerFaceAlpha', max(figure_cfg.simulation_marker_alpha, 0.5), ...
    'MarkerEdgeAlpha', max(figure_cfg.simulation_marker_alpha, 0.5));
...
function make_legend(ax, numerical_handles, fit_handles, ...
        experiment_handles, reference_handle, numerical_labels, fit_labels, ...
        experiment_labels, reference_label, figures_cfg)
%MAKE_LEGEND Assemble numerical points, fits, experiments, and reference.

font_cfg = figures_cfg.font;
legend_cfg = figures_cfg.legend;
legend_font_cfg = struct('family', legend_cfg.font_family, ...
    'fallback', legend_cfg.font_fallback);
legend_font_name = resolve_font(legend_font_cfg);
handles = [numerical_handles(:); fit_handles(:); experiment_handles(:); ...
    reference_handle(:)];
labels = [numerical_labels(:); fit_labels(:); experiment_labels(:); ...
    reference_label(:)];
if legend_cfg.box
    box_state = 'on';
else
...
legend_handle = legend(ax, handles, labels, ...
    'Interpreter', legend_cfg.interpreter, ...
    'FontName', legend_font_name, 'FontSize', font_cfg.legend_size, ...
    'FontWeight', legend_cfg.font_weight, ...
    'Location', legend_cfg.location, ...
    'NumColumns', legend_cfg.columns, ...
    'Orientation', legend_cfg.orientation, 'Box', box_state);
set(legend_handle, 'FontName', legend_font_name, ...
    'FontWeight', legend_cfg.font_weight, 'FontAngle', 'normal');
...
function color = marker_face_color(value)
%MARKER_FACE_COLOR Accept a transparent face or a configured hex color.
