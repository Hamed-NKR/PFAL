function [yfit, xfit, bounds_yfit, afit, bounds_afit] =...
    BAYESFIT_POLY3(x, y, group, resol)
% BAYESFIT perfroms a thrid-order polynomial regression in log-log...
...
X = [log_x, log_x.^2, log_x.^3, group];  % a third-order...
    % ...polynomial ensures a balance between capturing the curvature...
...
mu_group = mean(group);
X_fit = [ones(size(log_xfit)), log_xfit, log_xfit.^2, log_xfit.^3,...
...
for i = 1 : n_samples
    alpha_samples(:, i) = 3 + b1(i) + 2 * b2(i) * log_xfit +...
        3 * b3(i) * log_xfit.^2; % adding 3 to convert to Dm
end
