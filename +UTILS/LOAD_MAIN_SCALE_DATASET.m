function [data, dataset_src] = LOAD_MAIN_SCALE_DATASET(dataset_cfg)
%LOAD_MAIN_SCALE_DATASET Load the LD1 source library used by main_scale.
%   DATA = UTILS.LOAD_MAIN_SCALE_DATASET(DATASET_CFG) loads the MAT-file
%   variable declared in the dataset section of the main_scale config.
%
%   [DATA, DATASET_SRC] = UTILS.LOAD_MAIN_SCALE_DATASET(...) also returns
%   the normalized dataset metadata that was used during the load.

if nargin < 1 || ~isstruct(dataset_cfg)
    error('PFAL:LOAD_MAIN_SCALE_DATASET:MissingDatasetConfig', ...
        'The scale dataset configuration must be provided as a struct.');
end
...
if ~isfield(loaded, dataset_src.variable)
    error('PFAL:LOAD_MAIN_SCALE_DATASET:MissingVariableInFile', ...
        'Variable "%s" was not found in scale dataset file: %s', ...
...
if ~isfield(src, field_name) || isempty(src.(field_name))
    error('PFAL:LOAD_MAIN_SCALE_DATASET:MissingField', ...
        'The scale dataset configuration is missing the %s.', field_label);
end
...
if isempty(dataset_src.url)
    error('PFAL:LOAD_MAIN_SCALE_DATASET:MissingFileOnDisk', ...
        ['Dataset file for "%s" was not found: %s\n' ...
        'Add the file locally or set dataset.url in the scale config.'], ...
...
try
    ...
    error('PFAL:LOAD_MAIN_SCALE_DATASET:DownloadFailed', ...
...
%DEFAULT_CACHE_FILE Put remote MAT downloads in data/main_scale by default.

[~, file_name, file_ext] = fileparts(configured_file);
dataset_file = fullfile(repo_root(), 'data', 'main_scale', ...
...
if ~strcmpi(actual_sha256, expected_sha256)
    error('PFAL:LOAD_MAIN_SCALE_DATASET:ChecksumMismatch', ...
...
if fid < 0
    error('PFAL:LOAD_MAIN_SCALE_DATASET:HashReadFailed', ...
