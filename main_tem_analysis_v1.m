...
comparison_summary_tables = cell(n_entries, 1);
...
for i = 1:n_entries
    ...
    comparison_summary_tables{i} = ...
        entry_result_cells{i}.statistics.comparison_summary;
end
...
experimental_condition_comparison = ...
    vertcat_nonempty(comparison_summary_tables);
...
write_optional_table(experimental_condition_comparison, fullfile( ...
    cfg_tem.outputs.data_root, cfg_tem.outputs.comparison_summary_csv))
...
statistics = calculate_entry_statistics(entry, aggregate_rows, ...
    morphology_rows, primary_rows, analysis_cfg);
summary_table = statistics.summary_table;
...
entry_result.statistics = statistics;
...
function statistics = calculate_entry_statistics(entry, aggregate_table, ...
    morphology_table, primary_table, analysis_cfg)

% Compute condition-level statistics once and reuse the stored results for
% legacy exports, figure audit tables, and the experimental comparison.
...
[GM_da_morphology, GSD_da_morphology, CI_da_morphology] = ...
    UTILS.GEOMSTATS(morphology_table.da_nm);

statistics.summary_table = table( ...
    {entry.id}, {entry.label}, height(aggregate_table), ...
    height(morphology_table), height(primary_table), ...
    analysis_cfg.coverage_threshold, analysis_cfg.logistic_bandwidth, ...
...
statistics.ensemble_primary_summary = table( ...
    repmat({entry.id}, 3, 1), repmat({entry.label}, 3, 1), ...
    {'unweighted'; 'hard_coverage_weighted'; 'soft_coverage_weighted'}, ...
    repmat(height(primary_table), 3, 1), ...
    [GM_dpp_ens_unweighted; GM_dpp_ens_hard; GM_dpp_ens_soft], ...
    [GSD_dpp_ens_unweighted; GSD_dpp_ens_hard; GSD_dpp_ens_soft], ...
    [CI_dpp_ens_unweighted(1); CI_dpp_ens_hard(1); CI_dpp_ens_soft(1)], ...
    [CI_dpp_ens_unweighted(2); CI_dpp_ens_hard(2); CI_dpp_ens_soft(2)], ...
    'VariableNames', {'entry_id', 'entry_label', 'stat_mode', ...
    'n_primaries', 'GM_dpp', 'GSD_dpp', 'CI95_low_dpp', ...
    'CI95_high_dpp'});

statistics.per_aggregate_dpp_summary = aggregate_summary_row(entry, ...
    'per_aggregate_mean_dpp', height(aggregate_table), GM_dbarpp, ...
    GSD_dbarpp, CI_dbarpp);
statistics.per_aggregate_sigmapp_summary = aggregate_summary_row(entry, ...
    'per_aggregate_sigmapp', height(aggregate_table), GM_sigmapp, ...
    GSD_sigmapp, CI_sigmapp);

soft_n_eff = effective_sample_size(primary_table.weight_soft, ...
    primary_table.dpp_nm);
confidence_level = analysis_cfg.summary.confidence_level;
CI_dpp_ens_soft_comparison = configured_geometric_ci( ...
    GM_dpp_ens_soft, GSD_dpp_ens_soft, soft_n_eff, confidence_level, ...
    CI_dpp_ens_soft);
CI_dbarpp_comparison = configured_geometric_ci(GM_dbarpp, GSD_dbarpp, ...
    height(aggregate_table), confidence_level, CI_dbarpp);
CI_sigmapp_comparison = configured_geometric_ci(GM_sigmapp, ...
    GSD_sigmapp, height(aggregate_table), confidence_level, CI_sigmapp);
CI_da_morphology_comparison = configured_geometric_ci(GM_da_morphology, ...
    GSD_da_morphology, nnz(isfinite(morphology_table.da_nm) & ...
    morphology_table.da_nm > 0), confidence_level, CI_da_morphology);

comparison_rows = cell(9, 1);
comparison_rows{1} = geometric_comparison_row(entry, ...
    'projected_area_diameter', 'Projected-area-equivalent diameter', 'nm', ...
    nnz(isfinite(morphology_table.da_nm) & morphology_table.da_nm > 0), ...
    GM_da_morphology, GSD_da_morphology, confidence_level, ...
    CI_da_morphology_comparison);
comparison_rows{2} = morphology_comparison_row(entry, 'circularity', ...
    'Circularity', '1', morphology_table.ca, analysis_cfg);
comparison_rows{3} = morphology_comparison_row(entry, ...
    'relative_optical_depth', 'Relative optical depth', '1', ...
    morphology_table.zbar_opt, analysis_cfg);
comparison_rows{4} = morphology_comparison_row(entry, 'boundary_sharpness', ...
    'Boundary sharpness', '1', morphology_table.sbar_opt, analysis_cfg);
comparison_rows{5} = geometric_comparison_row(entry, ...
    'ensemble_primary_particle_diameter', ...
    'Bias-corrected ensemble primary-particle diameter', 'nm', ...
    height(primary_table), GM_dpp_ens_soft, GSD_dpp_ens_soft, ...
    confidence_level, CI_dpp_ens_soft_comparison);
comparison_rows{6} = geometric_comparison_row(entry, ...
    'per_aggregate_primary_particle_diameter', ...
    'Per-aggregate primary-particle diameter', 'nm', ...
    height(aggregate_table), GM_dbarpp, GSD_dbarpp, confidence_level, ...
    CI_dbarpp_comparison);
comparison_rows{7} = geometric_comparison_row(entry, ...
    'within_aggregate_primary_particle_spread', ...
    'Within-aggregate primary-particle spread', '1', ...
    height(aggregate_table), GM_sigmapp, GSD_sigmapp, confidence_level, ...
    CI_sigmapp_comparison);
comparison_rows{8} = morphology_comparison_row(entry, ...
    'subaggregate_count', 'Sub-aggregate count', '1', ...
    morphology_table.n_subagg, analysis_cfg, true);
comparison_rows{9} = frequency_comparison_row(entry, ...
    'multiple_subaggregate_frequency', ...
    'Aggregates with multiple sub-aggregates', '%', ...
    morphology_table.n_subagg, analysis_cfg);
statistics.comparison_summary = vertcat(comparison_rows{:});

end

function summary = aggregate_summary_row(entry, metric, n_values, gm, gsd, ci)

summary = table({entry.id}, {entry.label}, {metric}, n_values, gm, gsd, ...
    ci(1), ci(2), 'VariableNames', {'entry_id', 'entry_label', ...
    'metric', 'n_aggregates', 'GM_value', 'GSD_value', ...
    'CI95_low_value', 'CI95_high_value'});

end

function row = geometric_comparison_row(entry, metric_id, metric_label, ...
    unit, n_values, gm, gsd, confidence_level, ci)

row = comparison_row(entry, metric_id, metric_label, unit, n_values, ...
    'geometric_mean', gm, 'geometric_standard_deviation', gsd, NaN, NaN, ...
    'log_normal', confidence_level, ci(1), ci(2));

end

function row = morphology_comparison_row(entry, metric_id, metric_label, ...
    unit, values, analysis_cfg, require_positive_integer)
...
if require_positive_integer
    valid = valid & values >= 1 & values == round(values);
end
values = values(valid);
[estimate, spread, spread_low, spread_high, ci] = ...
    summarize_morphology_values(values, analysis_cfg);
row = comparison_row(entry, metric_id, metric_label, unit, numel(values), ...
    analysis_cfg.summary.morphology_center, estimate, ...
    analysis_cfg.summary.morphology_spread, spread, spread_low, ...
    spread_high, analysis_cfg.summary.morphology_interval, ...
    analysis_cfg.summary.confidence_level, ci(1), ci(2));

end

function row = frequency_comparison_row(entry, metric_id, metric_label, ...
    unit, values, analysis_cfg)

values = double(values(:));
values = values(isfinite(values) & values >= 1 & values == round(values));
n_values = numel(values);
count = nnz(values > 1);
[estimate, ci] = wilson_percentage(count, n_values, ...
    analysis_cfg.summary.confidence_level);
row = comparison_row(entry, metric_id, metric_label, unit, n_values, ...
    'percentage', estimate, 'none', NaN, NaN, NaN, ...
    analysis_cfg.summary.frequency_interval, ...
    analysis_cfg.summary.confidence_level, ci(1), ci(2));

end

function row = comparison_row(entry, metric_id, metric_label, unit, ...
    n_values, estimate_type, estimate, spread_type, spread, spread_low, ...
    spread_high, ci_method, confidence_level, ci_low, ci_high)

row = table({entry.id}, {entry.label}, {metric_id}, {metric_label}, {unit}, ...
    n_values, {estimate_type}, estimate, {spread_type}, spread, spread_low, ...
    spread_high, {ci_method}, confidence_level, ci_low, ci_high, ...
    'VariableNames', {'entry_id', 'entry_label', 'metric_id', ...
    'metric_label', 'unit', 'n', 'estimate_type', 'estimate', ...
    'spread_type', 'spread', 'spread_low', 'spread_high', 'ci_method', ...
    'confidence_level', 'ci_low', 'ci_high'});

end

function [estimate, spread, spread_low, spread_high, ci] = ...
    summarize_morphology_values(values, analysis_cfg)
...
estimate = summary_center(values, analysis_cfg.summary.morphology_center);
switch analysis_cfg.summary.morphology_spread
    case 'iqr'
        quartiles = prctile(values, [25 75]);
        spread = NaN;
        spread_low = quartiles(1);
        spread_high = quartiles(2);
...
bootstrap_center = zeros(analysis_cfg.random_resample_count, 1);
n_values = numel(values);
for i = 1:analysis_cfg.random_resample_count
    sample = values(randi(n_values, n_values, 1));
    bootstrap_center(i) = summary_center(sample, ...
        analysis_cfg.summary.morphology_center);
end
alpha = 1 - analysis_cfg.summary.confidence_level;
ci = prctile(bootstrap_center, 100 * [alpha / 2, 1 - alpha / 2]);

end

function value = summary_center(values, center_type)
...
function [percentage, ci] = wilson_percentage(count, n_values, confidence_level)
...
z = sqrt(2) * erfinv(confidence_level);
p = count / n_values;
denominator = 1 + z ^ 2 / n_values;
center = (p + z ^ 2 / (2 * n_values)) / denominator;
half_width = z * sqrt(p * (1 - p) / n_values + ...
    z ^ 2 / (4 * n_values ^ 2)) / denominator;
percentage = 100 * p;
ci = 100 * [max(0, center - half_width), min(1, center + half_width)];

end

function ci = configured_geometric_ci(gm, gsd, n_eff, confidence_level, ...
    default_ci)

if abs(confidence_level - 0.95) < 10 * eps
    ci = default_ci;
    return
end
if ~isfinite(gm) || ~isfinite(gsd) || n_eff <= 1
    ci = [gm gm];
    return
end
z = sqrt(2) * erfinv(confidence_level);
se = log(gsd) / sqrt(n_eff);
ci = exp(log(gm) + z * [-1 1] * se);

end

function n_eff = effective_sample_size(weights, values)
...
plot_artifacts.ensemble_primary_summary = ...
    collect_statistics_table(entry_results, 'ensemble_primary_summary');
plot_artifacts.per_aggregate_dpp_summary = collect_statistics_table( ...
    entry_results, 'per_aggregate_dpp_summary');
plot_artifacts.per_aggregate_sigmapp_summary = ...
    collect_statistics_table(entry_results, 'per_aggregate_sigmapp_summary');
...
set(ax, ...
    ...
    'YMinorGrid', logical_to_on_off(grid_cfg.y_minor), ...
    'YMinorTick', logical_to_on_off(grid_cfg.y_minor), ...
...
function summary_table = collect_statistics_table(entry_results, field_name)

summary_cells = cell(numel(entry_results), 1);
for i = 1:numel(entry_results)
    summary_cells{i} = entry_results(i).statistics.(field_name);
end
summary_table = vertcat_nonempty(summary_cells);
