...
% Load configurable inputs for the scale workflow from the local JSON
% registry. User-tuned defaults are kept out of the MATLAB source while the
% scaling/filtering algorithm below remains unchanged.
cfg_scale = UTILS.LOAD_MAIN_SCALE_CONFIG;
cfg_dataset = cfg_scale.dataset;
cfg_uc = cfg_scale.correlation.universal;
cfg_bc = cfg_scale.correlation.brasil;
cfg_dist = cfg_scale.distribution;
cfg_sampling = cfg_scale.sampling;
cfg_projection = cfg_scale.projection;
cfg_options = cfg_scale.options;

% Olfert and Rogak (2019)'s universal correlation
D_TEM = cfg_uc.D_TEM; % exponent
dpp100 = cfg_uc.dpp100; % prefactor
da_lim_uc = cfg_uc.da_lim_uc;  % limits on the projected area diameter (for plotting)
n_da_uc = cfg_uc.n_da_uc; % plot data counts
...
alpha_a = cfg_bc.alpha_a; % exponent
k_a = cfg_bc.k_a; % prefactor
npp_lim_bc = cfg_bc.npp_lim_bc; % limits on the number of primaries (for plotting)
n_npp_uc = cfg_bc.n_npp_uc; % plot data counts
...
gm_da = cfg_dist.gm_da; % geometric mean
gsd_da = cfg_dist.gsd_da; % geometric standard deviation

gm_dpp = cfg_dist.gm_dpp; % ensemble geometric mean of primary particle diameter distribution
gsd_dpp = cfg_dist.gsd_dpp; % ensemble geometric standard deviation

cn_scale = cfg_sampling.cn_scale; % proportion of random aggregates chosen for filtering

% resolution for projected area calculation
n_mc = cfg_projection.n_mc;
n_ang = cfg_projection.n_ang;

n_bin_filter = cfg_sampling.n_bin_filter;

%% Raw data against the Brasil's and universal correlations %%

% Load non-scaled first-stage library data through the scale config.
pars_raw.pp = UTILS.LOAD_MAIN_SCALE_DATASET(cfg_dataset);

% correct the structure if necessary
if iscell(pars_raw.pp)
    ...
    % Merge pp info from different times only when the imported library has
    % the extra historical nesting layer.
    while ~isempty(pars_raw.pp) && all(cellfun(@iscell, pars_raw.pp))
    ...
    error('PFAL:main_scale_v2:InvalidDatasetShape', ...
        'Scale dataset variable must be a cell array of aggregate primary-particle matrices.')
end
...
% The resolved config metadata is not used beyond this point.
clear cfg_scale cfg_dataset cfg_uc cfg_bc cfg_dist cfg_sampling cfg_projection cfg_options
...
% Store the scaled aggregate library with source-specific naming so scale
% and scatter outputs can coexist without overwriting one another.
repo_dir = fileparts(mfilename('fullpath'));
dir_scale_data = fullfile(repo_dir, 'data', 'main_scale');
if ~isfolder(dir_scale_data)
    mkdir(dir_scale_data)
end

save(fullfile(dir_scale_data, 'scaled_aggs_for_LD2_from_main_scale.mat'), ...
    'pars_out', '-v7.3')
...
% Keep scale plots in a scale-specific result folder.
dir_scale_results = fullfile(repo_dir, 'results', 'main_scale');
if ~isfolder(dir_scale_results)
    mkdir(dir_scale_results)
end

exportgraphics(f1, fullfile(dir_scale_results, 'raw.png'),...
    'BackgroundColor','none', 'ContentType','vector', 'Resolution', 300)
exportgraphics(f2, fullfile(dir_scale_results, 'scaled_filtered.png'),...
    'BackgroundColor','none', 'ContentType','vector', 'Resolution', 300)
exportgraphics(f3, fullfile(dir_scale_results, 'render.png'),...
    'BackgroundColor','none', 'ContentType','vector', 'Resolution', 300)
