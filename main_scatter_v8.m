...
% registry. All user-tuned defaults are centralized there so the script can
% be re-run without editing the MATLAB source.
cfg_scatter = UTILS.LOAD_MAIN_SCATTER_CONFIG;

% Split the configuration into sections so the physical inputs below map
% clearly to the JSON structure that defines them.
cfg_dataset = cfg_scatter.dataset;
cfg_uc = cfg_scatter.correlation.universal;
cfg_bc = cfg_scatter.correlation.brasil;
cfg_dist = cfg_scatter.distribution;
cfg_sampling = cfg_scatter.sampling;
cfg_projection = cfg_scatter.projection;
cfg_options = cfg_scatter.options;

% Olfert and Rogak (2019)'s universal correlation
D_TEM = cfg_uc.D_TEM; % exponent
dpp100 = cfg_uc.dpp100; % prefactor
da_lim_uc = cfg_uc.da_lim_uc; % projected-area diameter limits for plotting
n_da_uc = cfg_uc.n_da_uc; % number of plotting samples for the universal curve
...
npp_lim_bc = cfg_bc.npp_lim_bc; % primary-particle-count limits for plotting
n_npp_uc = cfg_bc.n_npp_uc; % number of plotting samples for the Brasil curve
...
gm_da = cfg_dist.gm_da; % geometric mean of projected area diameter distribution
gsd_da = cfg_dist.gsd_da; % geometric standard deviation of projected area diameter
gm_dpp = cfg_dist.gm_dpp; % geometric mean of primary particle diameter distribution
gsd_dpp = cfg_dist.gsd_dpp; % geometric standard deviation of primary particle diameter

cn_scat = cfg_sampling.cn_scat; % fraction of aggregates chosen for bivariate sampling
...
% Load the source LD1 aggregate library through the local registry. This
% keeps machine-specific file paths out of the script while preserving a
% stable dataset identifier in version control.
pars_raw.pp = UTILS.LOAD_SCATTER_LD1_DATASET(cfg_dataset);
...
% Count aggregates and primary particles before the sizing pass so the
% imported cell array is converted into the structure expected below.
n_agg_raw = length(pars_raw.pp); % number of aggregates in the imported library
pars_raw.n = zeros(n_agg_raw,1); % preallocate primary-particle counts
for i = 1 : n_agg_raw
    pars_raw.n(i) = size(pars_raw.pp{i}, 1); % primaries stored in aggregate i
end
...
lgd2b_scat = strcat(string(newline), 'Gaussian noise seeds (in log-log space)',...
    string(newline), '$\overline{d}_\mathrm{a}$ =', {' '}, num2str(gm_da, '%.0f'),...
    ',', {' '}, '$\sigma_\mathrm{a}$ =', {' '}, num2str(gsd_da, '%.1f'),...
    string(newline), '$\overline{d}_\mathrm{pp}$ =', {' '}, num2str(gm_dpp, '%.0f'),...
    ',', {' '}, '$\sigma_\mathrm{pp}$ =', {' '}, num2str(gsd_dpp, '%.1f'));
...
% if strcmp(opts_nppcor, 'on') || strcmp(opts_nppcor, 'On') ||...
%         strcmp(opts_nppcor, 'ON')
