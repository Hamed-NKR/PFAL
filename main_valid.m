...
fdir_simul_10_scat = 'D:\Hamed\CND\PhD\Publication\DLCA2\outputs\postLD2-28-Mar-2025_18-57-18_LD2-25-Nov-2024_19-37-48_Final';
fname_simul_10_scat = 'Post_LD2-25-Nov-2024_19-37-48_Final';
fdir_simul_10_flat = 'D:\Hamed\CND\PhD\Publication\DLCA2\outputs\postLD2-28-Mar-2025_18-59-48_LD2-26-Nov-2024_23-56-35_Final';
...
ii0 = [1,4,5]; % data group indices in simulation data to be plotted
...
[bayesfit_valid(1).yfit, bayesfit_valid(1).xfit, bayesfit_valid(1).bounds_yfit,...
    ...
    UTILS.BAYESFIT_POLY2(1e9 * parsdata_13_scat(1).dm,...
...
[bayesfit_valid(2).yfit, bayesfit_valid(2).xfit, bayesfit_valid(2).bounds_yfit,...
    ...
    UTILS.BAYESFIT_POLY2(1e9 * cat(1, parsdata_13_scat(ii0(2:end)).dm),...
    cat(1, parsdata_13_scat(ii0(2:end)).rho_eff), cat(1, parsdata_13_scat(ii0(2:end)).r_n_agg),...
    bayesresol);
...
[bayesfit_valid(3).yfit, bayesfit_valid(3).xfit, bayesfit_valid(3).bounds_yfit,...
    ...
    UTILS.BAYESFIT_POLY2(dist_grp(1).d_mode', dist_grp(1).rho_eff',...
...
[bayesfit_valid(4).yfit, bayesfit_valid(4).xfit, bayesfit_valid(4).bounds_yfit,...
    ...
    UTILS.BAYESFIT_POLY2(dist_grp(3).d_mode', dist_grp(3).rho_eff',...
...
ylabel('$\rho_\mathrm{eff} \mathrm{[kg/m^3]}$', 'interpreter', 'latex',...
    'FontSize', 24)
legend(cat(1, plt3{1:5,1}), legtxt3, 'interpreter', 'latex',...
...
fill([bayesfit_valid(1).xfit; flipud(bayesfit_valid(1).xfit)],...
    [bayesfit_valid(1).bounds_yfit(:,1); flipud(bayesfit_valid(1).bounds_yfit(:,2))],...
    hex2rgb('#9F5255'), 'EdgeColor', 'none', 'FaceAlpha', 0.2);
...
fill([bayesfit_valid(2).xfit; flipud(bayesfit_valid(2).xfit)],...
    [bayesfit_valid(2).bounds_yfit(:,1); flipud(bayesfit_valid(2).bounds_yfit(:,2))],...
    hex2rgb('#006A71'), 'EdgeColor', 'none', 'FaceAlpha', 0.2);
...
plt5{4} = scatter(dist_grp(3).d_mode, dist_grp(3).rho_eff,...
    45, hex2rgb('#659287'), 'h', 'LineWidth', 1.5); % high agglomeration
